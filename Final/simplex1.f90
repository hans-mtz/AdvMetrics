...
CONTAINS
  SUBROUTINE Nelder_Meade1(x,ftol,func,id,itmax)
      ...
      DO
         ...
         if (rtol < ftol) then
            ...
            IF (id==0) WRITE(*,*) "Stopped by tolerance"
						if (id==3) print*, "Stopped by tolerance"
            ! print*, id,"Stopped by tolerance"
            !RETURN
            EXIT
         end if
         IF (iter >= ITMAX) THEN
