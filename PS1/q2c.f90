...
contains
    ...
    subroutine q2c()
      ...
      ! print*, "Sample is = ",(x(i), i=1,4)
      int_r=0.0D0
      simsloop: do i=1,size(sim)
          ns=sim(i)

          simulating: do j=1,ns
              ! call SET_SEED(j+5*j,j**3,j*2,j)

              x=Sample_Multivariate_Normal(miu,varcovar)

              if (x(1)>x(2) .and. x(1)>x(3) .and. x(1)>x(4)) then
                int_r(i)=int_r(i)+1.0d0
            endif !indicator
