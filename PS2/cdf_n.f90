module cdf_m
    ...
    USE INTEGRATION, only : gauss_chebyshev
    use probability, only : pdf_normal

  IMPLICIT NONE

contains

  function cdf_n(u,miu,sigma) result(res)
      real(8), INTENT(IN) :: u, miu, sigma
      real(8), INTENT(OUT) :: res
      INTEGER, PARAMETER :: n=20
      integer :: i
      real(8), :: quadx(n),quadw(n), drho,W,v,t,f,g
      CALL GAUSS_CHEBYSHEV(quadx,quadw)
      ! u=a(k,i,j)
      res=0.0d0
      DO i=1,n
          drho= 1.0d0/(1.0d0+quadx(i))
          ! drho= (2.0d0*u)/((1.0d0+quadx(h))**2.0d0)
          W=SQRT(1.0d0-quadx(i)*quadx(i))
          v=quadw(i)*drho*W/W
          t=LOG(1.0d0 + quadx(i))-LOG(2.0d0)+u
          ! t=(2.0d0*u*quadx(h))/(1.0d0+quadx(h))
          f=PDF_NORMAL(t,miu(i),sigma(j))
          g=f*W
          res= res + g*v
      END DO
  end function cdf_n
